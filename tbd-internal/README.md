...
```
tbd-internal/
├── tbd-internal-war/          # Web application module
│   ├── src/main/
│   │   ├── java/              # Java source code
│   │   ├── resources/         # Configuration files
│   │   └── webapp/            # Web resources (JSP, etc.)
│   └── pom.xml
├── tbd-internal-ear/          # EAR packaging module
│   └── pom.xml
...
This creates:
- `tbd-internal-war/target/tbd-internal.war`
- `tbd-internal-ear/target/tbd-internal.ear`
...
4. Test your changes at http://localhost:9080/internal
...
This builds an **immutable image** with the EAR baked in - exactly what goes to ECS.
...
- Open IntelliJ IDEA
- File → Open → Select `tbd-internal` directory
...
3. Host: `localhost`
4. Port: `7777`
5. Use module classpath: `tbd-internal-war`
...
### Web Pages (Struts)
- `http://localhost:9080/internal/` - Home page
- `http://localhost:9080/internal/home.action` - Main application page

### REST APIs (Spring)
- `GET /internal/api/health` - Health check
- `GET /internal/api/db-test` - Database connectivity test
- `GET /internal/api/validate-ip?ip=<ip>` - IP validation
- `GET /internal/api/sample-data` - Sample database query
...
3. **Package**: Build Docker image and push to ECR
4. **Deploy**: Update ECS service (manual trigger)
...
docker build -t tbd-internal:prod -f docker/Dockerfile .

# Tag for ECR
docker tag tbd-internal:prod \
...
2. Check network connectivity:
   ```bash
   docker exec -it tbd-internal-dev ping oracle-db
   ```
